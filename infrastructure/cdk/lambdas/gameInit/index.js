...
class UnderTheSeaGameInit {
    ...
    getUserPoolId(callback) {
        if (this.UserPoolId == null) {
            var self = this;
            SSM.getParameters( { 'Names' : [ 'underthesea.userpoolid' ] }, (err,data) => {
                if (err) {
                    console.log('Error reading underthesea.userpoolid');
                    console.log(err);
                    callback(null);
                } else {
...
var gi = new UnderTheSeaGameInit();
gi.init();