...
/*
  ...
  the core logic for the Under the Sea game.
...
/*  
    ...
    The Game class represents an Under the Sea game.
...
function Game() {
    ...
    // load sprite
    this.invaderImg = new Sprite('./../images/fish.png', 1, 2);
...
function RegisterFailureState(err) {
    this.modal = new Modal(document.getElementById("modalDialog"));
    var modalDialogString =
        `<h2>UnderTheSea:REGISTER - #FAILURE#</h2>
         <h3>`+ err.message + `.</h3>
         Try again.`;
                    ...
                    if (game.session.Synchronized) {
                        ...
                        game.scoreboardInterval = setInterval( function() {
                            game.awsfacade.getScoreboard(game.session.SessionId,function(err,data) {
                                ...
                                wavefield.setScoreboard(scoreboard);
                            })
                        },2000);
                        game.run();
                    }
...
WelcomeState.prototype.draw = function (game, dt, ctx) {
    ...
    ctx.fillText("Under the Sea", game.width / 2, game.height / 2 - 40);
    ctx.font = "16px Arial";
    if (this.userCanPlay)
